  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      if (
        ...
        this.logger.error(
          'Please run "npx prisma generate" and restart the application',
        );
      } else if (
        ...
        this.logger.error(
          'Database server is not reachable. Please check your connection settings and ensure the database server is running.',
        );
      } else if (error.message?.includes('P1003')) {
        this.logger.error(
          'Database or schema does not exist. Please run "npx prisma migrate dev" to create it.',
        );
      }
